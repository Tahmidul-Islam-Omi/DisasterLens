...
BMD weather feeds     ──┤     Gemini summarize  ──►        ├─ Weather alerts
...
| Layer    | Technologies |
|----------|--------------|
| Backend  | FastAPI, MongoDB (Motor async), JWT auth (python-jose, PBKDF2), LangChain + Gemini, BeautifulSoup4 + httpx scraping, pypdf, deep-translator |
...
**Prerequisites:** Python 3.10+, Node.js 18+, MongoDB 4.4+ (local or Atlas), Google Gemini API key (optional — AI features degrade gracefully without it).
...
| Volunteer | `GET /api/v1/volunteer/dashboard` · `POST /api/v1/volunteer/coverage-updates` · `POST /api/v1/volunteer/infra-exposures` |
...
# AI (optional — extractive summaries are used without a key)
...
- ✅ Gemini summarization (BN/EN) with translation and heuristic fallback
- ✅ AI impact snapshots feeding the impact summary & geospatial dashboards
- ✅ Volunteer coverage updates (GPS), infrastructure exposure reports, vulnerable communities, missing-person reports
- ✅ Task / volunteer / alert / community CRUD for authorities
- ✅ Bilingual UI, PWA install + offline caching, role-gated routing

**In progress / planned**
- 🚧 Backend persistence for field reports, activity logs, community status, and authority registration (UI exists, endpoints pending)
- 🚧 Request validation (Pydantic schemas) and auth coverage for ingestion & coverage endpoints
- 🚧 Real AI alert simplification (current endpoint is a placeholder)

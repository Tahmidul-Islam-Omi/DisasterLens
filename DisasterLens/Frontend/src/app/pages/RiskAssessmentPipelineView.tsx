...
const riskData = [
  {
    ...
    exposure: 94,
    vulnerability: 88,
    priorityKey: 'severity_critical',
    driverKeys: ['driver_river_proximity', 'driver_poor_drainage', 'driver_high_density'],
    actionKey: 'action_evacuate_zone_a',
    exposedAssets: [
  ...
  },
  {
    ...
    exposure: 85,
    vulnerability: 76,
    priorityKey: 'severity_high',
    driverKeys: ['driver_coastal_surge', 'driver_aging_seawall'],
    actionKey: 'action_standby_evacuation',
    exposedAssets: [
  ...
  },
  {
    ...
    exposure: 78,
    vulnerability: 65,
    priorityKey: 'severity_medium',
    driverKeys: ['driver_chemical_plants', 'driver_floodplain'],
    actionKey: 'action_secure_hazmat',
    exposedAssets: [
  ...
  },
  {
    ...
    exposure: 45,
    vulnerability: 50,
    priorityKey: 'severity_low',
    driverKeys: ['driver_landslide_risk', 'driver_steep_terrain'],
    actionKey: 'action_monitor_slopes',
    exposedAssets: [
  ...
  },
  {
    ...
    vulnerability: 20,
    priorityKey: 'severity_low',
    driverKeys: ['driver_urban_flash_flood'],
    actionKey: 'action_clear_drains',
    exposedAssets: [
...
export function RiskAssessmentPipelineView() {
  ...
  const getPriorityColor = (priorityKey: string) => {
    switch (priorityKey) {
      case 'severity_critical': return 'bg-red-100 text-red-800 border-red-200';
      case 'severity_high': return 'bg-orange-100 text-orange-800 border-orange-200';
      case 'severity_medium': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
      case 'severity_low': return 'bg-green-100 text-green-800 border-green-200';
                ...
                <tbody className="divide-y divide-gray-100">
                  {riskData.map((row) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <span className={`px-2.5 py-1 rounded-md text-xs font-bold border ${getPriorityColor(row.priorityKey)}`}>
                          {t(row.priorityKey)}
                        </span>
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        <div className="flex flex-wrap gap-1">
                          {row.driverKeys.slice(0, 2).map(d => (
                            <span key={d} className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-[10px] font-semibold">
                              {t(d)}
                            </span>
                          ))}
                          {row.driverKeys.length > 2 && <span className="text-gray-400 text-[10px] font-bold">+{row.driverKeys.length - 2}</span>}
                        </div>
                      </td>
                      <td className="p-4 pr-6">
                        <span className="text-sm font-semibold text-gray-800 flex items-center gap-1">
                          {t(row.actionKey)} <ArrowRight className="w-3 h-3 text-gray-400" />
                        </span>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="p-6 flex flex-col gap-6 overflow-y-auto">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`px-2.5 py-1 rounded-md text-xs font-bold border ${getPriorityColor(selectedArea.priorityKey)}`}>
                    {t(selectedArea.priorityKey)}
                  </span>
                </div>
              ...
              <div className="w-full h-40 bg-slate-100 rounded-xl border border-gray-200 flex items-center justify-center relative overflow-hidden">
                ...
                <div className="bg-white/90 p-2 rounded-lg shadow-sm border border-gray-200 z-10 flex items-center gap-2 backdrop-blur-sm">
                  <MapPin className={`w-4 h-4 ${selectedArea.priorityKey === 'severity_critical' ? 'text-red-500' : 'text-blue-500'}`} />
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedArea.driverKeys.map((driverKey, idx) => (
              ...
              <div className="mt-auto pt-4 border-t border-gray-100">
                <button className={`w-full py-3 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors ${
                  selectedArea.priorityKey === 'severity_critical' ? 'bg-[#DC2626] hover:bg-red-700 text-white' : 
                  selectedArea.priorityKey === 'severity_high' ? 'bg-orange-600 hover:bg-orange-700 text-white' :
                  'bg-[#1E3A8A] hover:bg-blue-900 text-white'
                }`}>
                  {t(selectedArea.actionKey)} <ArrowRight className="w-4 h-4" />
                </button>
              </div>
